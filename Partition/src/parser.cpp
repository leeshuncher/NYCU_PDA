...
static bool next_token_fast(const char *&p, const char *end, string_view &tok, bool &is_semi, bool &ends_with_semi)
{
    skip_spaces(p, end);
    if (p >= end)
        return false;
...
static int ensure_cell_id(cell_id_map &cell_id,
                          vector<cell> &cells,
...
static void append_net_cell(net &n, int cid, bool fast_dedup, vector<int> &net_seen_stamp, int cur_stamp)
{
    if (!fast_dedup)
    {
...
static void finalize_net(parsed_input &out, net &n, bool fast_dedup)
{
    if (!fast_dedup)
    {
        ...
        n.connected_cells.erase(unique(n.connected_cells.begin(), n.connected_cells.end()), n.connected_cells.end());
    }
...
parsed_input parse_input(const string &filename)
{
    ...
    const size_t estimated_cells = max<size_t>(1 << 16, static_cast<size_t>(max<streamoff>(file_size / 32, 0)));
    const size_t estimated_nets = max<size_t>(1 << 14, static_cast<size_t>(max<streamoff>(file_size / 48, 0)));
